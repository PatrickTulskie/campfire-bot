  ...
  module Event
    ...
    class Command < EventHandler
      ...
      def match?(msg)
        (
          msg[:message][0..0] == '!' ||
          msg[:message]       =~ Regexp.new("^#{bot.config['nickname']}(,|:|\s)", Regexp::IGNORECASE)
        ) &&
        msg[:message].gsub(/^\!/, '').gsub(Regexp.new("^#{bot.config['nickname']}(,|:|\s)?\\s*", Regexp::IGNORECASE), '').split(' ')[0].to_s.downcase == @matcher.downcase
      ...
      def filter_message(msg)
        msg[:message] = msg[:message].gsub(Regexp.new("^(!|#{bot.config['nickname']}(,|:|\s)?\\s+)#{@matcher.downcase}\\s?", Regexp::IGNORECASE), '')
        msg
      end
    end
    ...
    class Message < EventHandler
      ...
      def match?(msg)
        # puts "(#{msg[:message] =~ @matcher}) #{msg[:message]} vs #{@matcher}"
        msg[:message] =~ @matcher
      end
    end
